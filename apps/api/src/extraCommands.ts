...
async function guard(c: Context, permission: Permission) {
...
async function resolveTarget(c: Context) {
  const reply = c.message?.reply_to_message?.from;
  if (reply) return { id: reply.id, name: fullName(reply) };
  const raw = String(c.match ?? "").trim().split(/\s+/)[0];
  if (!raw || !c.chat) return null;
...
export function installExtraCommands(bot: Bot) {
  ...
  bot.command("info", async (c) => {
    if (!c.from) return;
    if (!["group", "supergroup"].includes(c.chat.type)) return c.reply("Используйте команду в группе.");
  ...
  bot.command("reload", async (c) => {
    if (!(await guard(c, "ROLE_MANAGE"))) return;
    const admins = await c.api.getChatAdministrators(c.chat.id);
  ...
  bot.command("unban", async (c) => {
    if (!(await guard(c, "BAN"))) return;
    const target = await resolveTarget(c);
    if (!target) return c.reply("Ответьте на сообщение пользователя или укажите его ID.");
  ...
  bot.command("purge", async (c) => {
    if (!(await guard(c, "DELETE"))) return;
    const start = c.message?.reply_to_message?.message_id;
    const end = c.message?.message_id;
    if (!start || !end) return c.reply("Ответьте командой /purge на первое сообщение диапазона.");
    ...
    if (deleted) await c.reply(`<b>Очистка завершена</b>\n\nУдалено сообщений: ${deleted}`, { parse_mode: "HTML" });
  });
  bot.command("pin", async (c) => {
    if (!(await guard(c, "ANNOUNCE"))) return;
    ...
    if (!messageId) return c.reply("Ответьте на сообщение или добавьте текст после /pin.");
  ...
  bot.command("link", async (c) => {
    if (!["group", "supergroup"].includes(c.chat.type)) return;
    try {
